...
int status[M/32+2]={0};
int primes[5761456];
unsigned int precalary[5761456];
void sieve()
{
    int sqN=(int)sqrt(M);
    int i,j,cnt=1;
...
void precal()
{
    ...
    {
        precalary[i]=precalary[i-1]*primes[i];
    }
}

unsigned int find_ans(int n)
{
    unsigned int res=1;
    int tmp;
    for(int i=0;primes[i]*primes[i]<=n;i++)
    {
        ...
        //res=res*(unsigned int)pow(primes[i],(int)(log(n)/log(primes[i]))-1);
    }
    int pos=upper_bound(primes,primes+5761456,n)-primes;
    //cout<<precalary[pos-1]<<" ttttttt -- -- ";
    return res*precalary[pos-1];
}
