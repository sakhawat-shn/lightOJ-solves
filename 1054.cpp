...
vector<pair<LLD,LLD> > primeFactors(LLD n)
{
    vector<pair<LLD,LLD> > factors;
    LLD pf_idx=0;
    LLD pf=primes[pf_idx]; //primes has been populated by sieve
    while(pf*pf<=n)
    {
        if(n%pf==0)
        {
...
LLD calcu(LLD a,LLD i)
{
    LLD up=big_mod(a,i+1,MOD)-1;
    LLD down=big_mod(a-1,MOD-2,MOD);
    //cout<<"--------------"<<up<<"    "<<down;
    return (up*down)%MOD;
}

LLD SOD_mod(LLD n,LLD m)
{
    vector<pair<LLD,LLD> > factors=primeFactors(n);
    LLD sod=1;
    for(int i=0;i<factors.size();i++)
    {
        sod=(sod*(calcu(factors[i].first,m*factors[i].second)))%MOD;
        cout<<"SOD "<<i<<" "<<sod<<endl;
    }
    return (sod+MOD)%MOD;
}
