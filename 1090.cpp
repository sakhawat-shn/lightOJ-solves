...
LLD no_x_in_p_q(LLD p,LLD q,LLD who)
{
    LLD cnt=0;
    while(p%who==0)
    {
...
LLD no_x_in_fact(LLD n,LLD who)
{
    LLD cnt=0;
    for(LLD i=who;i<=n;i=i*who)
    {
        cnt=cnt+(n/i);
    }
    return cnt;
...
int main()
{
    ...
    for(int i=0;i<t;i++)
    {
        ...
        scan(p);
        scan(q);
        LLD upore2=no_x_in_fact(n,2)+no_x_in_p_q(p,q,2);
        //dbug("upore2");dbug(upore2);
        LLD upore5=no_x_in_fact(n,5)+no_x_in_p_q(p,q,5);
        //dbug("upore5");dbug(upore5);
        LLD nice2=no_x_in_fact(n-r,2)+no_x_in_fact(r,2);
        //dbug("nice2");dbug(nice2);
        LLD nice5=no_x_in_fact(n-r,5)+no_x_in_fact(r,5);
        //dbug("nice5");dbug(nice5);
        upore2=upore2>nice2?upore2-nice2:0;
        upore5=upore5>nice5?upore5-nice5:0;
        printf("Case %d: ",i+1);
        print(upore2>upore5?upore5:upore2);
        print(newline);
    }
